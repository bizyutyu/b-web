...
まず、PR body に `(?i)(closes|fixes|resolves)\s+#\s*$` にマッチする番号なし行（テンプレートの `Closes #` 等）があれば削除し、`gh pr edit <番号> --body "<新body>"` で適用する。

次に、PR body から `(?i)(closes|fixes|resolves)\s+#(\d+)` を全マッチで抽出。
...
- body にシングルクォートが含まれる可能性があるため、`printf '%s' '<新body>' > /tmp/gh_pr_body.txt && gh pr edit <番号> --body-file /tmp/gh_pr_body.txt` で適用
