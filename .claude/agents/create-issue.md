---
name: create-issue
description: タイトルと概要から GitHub Issue を新規作成し、ブランチ作成コマンドを提示する専任エージェント。Issue body にはチェックリスト付きテンプレートを使用。対象リポジトリは `bizyutyu/b-web` 固定。
tools: Read, Bash
model: sonnet
---
...
作成結果の URL（`https://github.com/bizyutyu/b-web/issues/<N>`）から Issue 番号を抽出する。
