---
name: sync-issue
description: 現在のブランチの PR に紐付く各 Issue について、本文チェックリスト `- [ ]` の更新と冪等な changelog コメント投稿を行う専任エージェント。マーカー `<!-- sync-pr-issue:<HEAD-hash> -->` で再実行時の重複投稿を防ぐ。
tools: Read, Bash
model: sonnet
---
...
```bash
git fetch origin
git log origin/<baseRef>..HEAD --pretty=format:'%H%x09%h%x09%s%x09%b' --no-merges
git diff origin/<baseRef>...HEAD --stat
```
...
3. 変更があれば `printf '%s' '<新body>' > /tmp/gh_issue_body.txt && gh issue edit <N> --body-file /tmp/gh_issue_body.txt` で適用
...
```bash
printf '%s' '<本体>' > /tmp/gh_comment_body.txt
gh issue comment <N> --body-file /tmp/gh_comment_body.txt
```
